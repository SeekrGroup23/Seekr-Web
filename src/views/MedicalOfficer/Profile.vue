              ...
              <v-card flat color="grey lighten-3">
                <v-container fluid>
                  <v-layout column>
                    ...
                    <v-flex>
                      <v-text-field
                        v-model="mo.doctorRegNo"
                        label="Medical Officer ID"
                        readonly
                      ></v-text-field>
                    </v-flex>
                    ...
                    <v-flex>
                      <v-text-field
                        v-model="mo.currentWorking_hospitalName"
...
export default {
  ...
  methods: {
    ...
    onFileSelected(event) {
      ...
      this.mo.imageURL = URL.createObjectURL(this.selectedFile);
    ...
    uploadFile() {
      ...
      this.$http
        .post(
          "/api/medical_officer/" + this.moID + "/profile_image",
          formData,
          {
    ...
    getProfile() {
      ...
      this.$http
        .get("/api/medical_officer/get_profile/" + this.moID)
        .then(res => {
          this.mo.firstName = res.data.firstName;
          this.mo.lastName = res.data.lastName;
          this.mo.nic = res.data.nic;
          this.mo.dob = res.data.dob;
          this.mo.perm_address = res.data.perm_address;
          this.mo.temp_address = res.data.temp_address;
          this.mo.personalTelNo = res.data.teleNum_Private;
          this.mo.gender = res.data.gender;
          this.mo.specialty = res.data.specialty;
          this.mo.email = res.data.email;
          this.mo.currentWorking_hospitalName =
            res.data.currentWorking_hospitalName;

          this.mo.doctorRegNo = res.data.doctorRegistrationNo;
          this.mo.specialty = res.data.specialty;
          this.mo.dateJoined = res.data.dateCreated.split("T")[0];
          this.mo.currentWorking_hospitalName =
            res.data.currentWorking_hospitalName;
          this.mo.email_official = res.data.email_official;
          this.mo.teleNum_official = res.data.teleNum_official;

          this.mo.imageURL = "";

          this.getImageFromServer(res.data.imageURL);
        })
        .catch();
    },
    onClickEdit() {
    ...
    savePersonalInfo() {
      ...
      this.$http
        .put("api/medical_officer/" + this.moID + "/personal", {
          firstName: this.mo.firstName,
          lastName: this.mo.lastName,
          nic: this.mo.nic,
          dob: this.mo.dob,
          perm_address: this.mo.perm_address,
          temp_address: this.mo.temp_address,
          teleNum_Private:
            this.mo.personalTelNo == "" ? "null" : this.mo.personalTelNo,
          gender: this.mo.gender
        })
        .then(res => {
          if (res.data.message == "Success") {
            console.log("Updated");
      ...
      this.$http
        ...
        .then(res => {
          if (res.data != null) {
            ...
            this.mo.image = "data:image/jpeg;base64, " + res.data.img;
          }
          console.log(res.data);
        })
        .catch();
  ...
  created() {
    this.moID = this.$store.state.user.id;
    this.getProfile();
  }
};
</script>
